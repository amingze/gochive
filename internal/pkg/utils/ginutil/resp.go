...
// JSONBad
func JSONBad(c *gin.Context, status int, code int, err error) {
	c.AbortWithStatusJSON(status, httputil.JSONResponse{
		Code: code,
		Msg:  err.Error(),
	})
...
// BadRequest
func JSONRequest(c *gin.Context, err error) {
	JSONError(c, http.StatusOK, err)
}

// BadRequest
func JSONErrRequest(c *gin.Context, err error) {
	JSONError(c, http.StatusBadRequest, err)
}

// BadRequest
func JSONBadRequest(c *gin.Context, err error) {
	JSONBad(c, http.StatusOK, 400, err)
}
